define([
    ...
    Hotkeys.prototype.mount = function () {
        ...
        if (this.state.hotkeysActive) { this._bindKeys(); }
    };
    ...
    Hotkeys.prototype._onChange = function () {
        ...
        if (this.state.hotkeysActive !== newState.hotkeysActive) {
            if (newState.hotkeysActive) { this._bindKeys(); } else { this._unbindKeys(); }
        }
    ...
    Hotkeys.prototype._betCashout = function () {
        var invalidBet = (StateLib.canUserBet(
            Engine.balanceSatoshis,
            ControlsStore.getBetSize(),
            ControlsStore.getExtraBetSize(),
            this.state.betInvalid,
            this.state.cashOutInvalid,
            Engine.nMinBetAmount,
            Engine.nMaxBetAmount,
            Engine.nMinExtraBetAmount,
            Engine.nMaxExtraBetAmount) instanceof Error);
        ...
        if (notPlaying || isBetting) {
            ...
            } else if (!invalidBet && !Engine.placingBet) { // Not Playing
                HotkeysActions.placeBet(StateLib.parseBet(ControlsStore.getBetSize()), StateLib.parseExtraBet(ControlsStore.getExtraBetSize()), StateLib.parseCashOut(ControlsStore.getCashOut()));
            }

        // The user is playing and is not cashing out already
        } else if (!Engine.cashingOut) { HotkeysActions.cashOut(); }
    };
