...
define([
    ...
    ScriptExecutor.prototype.getMaxBet = function () {
        return Engine.nMaxBetAmount * 100;
    };

    /**
     * Gets the minimum amount you can bet per game
     */
    ScriptExecutor.prototype.getMinBet = function () {
        return Engine.nMinBetAmount * 100;
    };
    ...
    ScriptExecutor.prototype.lastGamePlay = function () {
        if (this.lastGamePlayed()) {
            if (Engine.tableHistory[0].player_info[Engine.username].stopped_at) { return 'WON'; } else { return 'LOST'; }
        }
     ...
     */
    ScriptExecutor.prototype.placeBet = function (bet, extraBet, cashOut, callback) {
        // bet condition
        bet = parseInt(bet);
        if (isNaN(bet)) {
            console.log('bet should be integer.');
            return;
        }
        if (bet < 0) {
        ...
        if (bet < Engine.nMinBetAmount * 100) {
            console.log('bet should be greater than', Engine.nMinBetAmount, 'Bits.');
            return;
        }
        if (bet > Engine.nMaxBetAmount * 100) {
            console.log('bet should be smaller than', Engine.nMaxBetAmount, 'Bits.');
            return;
        }
        if (bet > Engine.balanceSatoshis) {
            console.log('bet should be smaller than', Engine.balanceSatoshis, 'Bits.');
            return;
        }
        ...
        if (isNaN(extraBet)) {
            console.log('extra-bet should be integer.');
            return;
        }
        if (extraBet < 0) {
            console.log('extra-bet should be greater than 0.');
            return;
        }
        if (extraBet > 0 && extraBet < Engine.nMinExtraBetAmount) {
            console.log('extra-bet should be greater than', Engine.nMinExtraBetAmount, 'Bits.');
            return;
        }
        if (extraBet > Engine.nMaxExtraBet) {
            console.log('extra-bet should be smaller than', Engine.nMaxExtraBet, 'Bits.');
            return;
        }
        if (extraBet > Engine.balanceSatoshis) {
            console.log('extra-bet should be smaller than', Engine.balanceSatoshis, 'Bits.');
            return;
        }

        cashOut = parseInt(cashOut);
        if (isNaN(cashOut)) {
            console.log('cashOut should be float-number.');
            return;
        }

        if (cashOut < 0) {
            console.log('cashOut should be greater than 0.');
            return;
        }

        if (cashOut < 1) {
            console.log('cashOut should be greater than 1.');
            return;
        }

        Engine.bet(bet, extraBet, cashOut, callback);
    };
