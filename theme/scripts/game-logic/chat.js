define([
    ...
    function Chat () {
        ...
        self.ws.on('msg', function (data) {
            ...
            if (self.history.length > AppConstants.Chat.MAX_LENGTH) { self.history.splice(-100, 100); }

            // Match @username until end of string or invalid username char
            // var r = new RegExp('@' + self.username + '(?:$|[^a-z0-9_\-])', 'i');
            // var bMsgValid = r.test(data.message);
            if (data.type === 'say' && data.username !== self.username && data.message != null && data.message.length != 0 && enableChatMsg === true) {
                new Audio('/sounds/chatmsg.mp3').play();
            }
        ...
        self.ws.on('join', function (info) {
            ...
            if(self.state == 'JOINED' || self.state == 'CONNECTED')
    ...
    Chat.prototype.listMutedUsers = function (ignoredClientList) {
        ...
        if (ignoredClientListArr.length === 0) { ignoredListMessage = 'No users ignored'; } else {
            ignoredClientListArr.forEach(function (key, index) {
                if (index !== 0) { ignoredListMessage += ' ,' + ignoredClientList[key].username; } else { ignoredListMessage += ignoredClientList[key].username; }
            });
        }
