define([
    ...
    return React.createClass({
        ...
        render: function () {
            if (this.state.engine.gameState === 'STARTING') { return D.div({ className: 'bet-bar-starting' }); }
    ...
    function calculatePlayingPercentages (engine) {
        ...
        var bitsPlaying, bitsCashedOut, bitsCashedOutAfterMe;
        bitsPlaying = 0;
        bitsCashedOut = 0;
        bitsCashedOutAfterMe = 0;
        ...
        _.forEach(engine.playerInfo, function (player, username) {
            if (username !== engine.username) {
                if (player.stopped_at) {
                    if (player.stopped_at > myStop) { bitsCashedOutAfterMe += player.bet; } else { bitsCashedOut += player.bet; }
                } else {
