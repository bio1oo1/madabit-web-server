    ...
    return React.createClass({
        ...
        render: function () {
            ...
            if (ChatEngine.state == 'JOINED') {
                var messages = [];
                for (var i = ChatEngine.history.length - 1; i >= 0; i--) { messages.push(this._renderMessage(ChatEngine.history[i], i)); }
                ...
                edtInput = D.input(
                    {
                        ...
                        ref: 'input',
                        'type': 'text',
                        'placeholder': languageFlag ? 'Type a message here...' : '输入你的信息'
                    });

                btnSend = D.div({ className: 'btn-cont' },
                    // D.span({ className: 'arrow'}),
                    D.a({className: 'btn blue icn-only', onClick: this._clickSendButton, style:{boxShadow:'none'}},
                ...
                edtInput = D.input(
                    {
                        ...
                        className: 'form-control',
                        ref: 'input',
                        placeholder: languageFlag ? 'Log in to chat...' : '登陆聊',
                        disabled: true
                    });

                btnSend = D.div({ className: 'btn-cont', 'disabled': 'disabled' },
                    D.span({ className: 'arrow', 'disabled': 'disabled'}),
                    D.a({className: 'btn blue icn-only', 'disabled': 'disabled', style:{boxShadow:'none'}},
