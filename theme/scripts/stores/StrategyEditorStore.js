...
define([
    ...
    for (var strategy in Strategies) {
        if (Strategies[strategy].isWidget) { _widgetStates[strategy] = Strategies[strategy]['initialState']; }
    }
    ...
    var StrategyEditorStore = _.extend({}, Events, {
        ...
        _runStrategy: function () {
            ...
            if (_strategy.isWidget) { logicFn = _strategy.logic(_widgetStates[_selectedStrategy]); }
            // else is a string and we convert that string in to a function
            else { logicFn = new Function('engine', _strategy); }
        ...
        _validateInput: function () {
            if (_strategy.isWidget) { _widgetStates[_selectedStrategy].invalidData = _strategy.validate(_widgetStates[_selectedStrategy]); } else { _widgetStates[_selectedStrategy].invalidData = false; }
        },
