...
function Chat (io) {
    ...
    function onConnection (socket) { // socket.user is attached on the login middleware
        ...
        socket.on('disconnect', function () {
            ...
            var strUser = '';
            if (socket.user) {
                strUser = socket.user.username;
                db.setPlaying(socket.user.username, false, function (err) {
                    // console.log("[", socket.user.username, "] is free for login.");
                });
            }

            console.log('W: ' + isoDate + ' - ' + strIP + ':' + strUser + '  online:' + self.clientCount);
        });

        // Join to a Room
        socket.on('join', function (channelName) {
            // debug('join event received from user %s', socket.user ? socket.user.username : socket.handshake.address.address);
        ...
        if (socket.user) {
            strUser = socket.user.username;
            db.setPlaying(socket.user.username, true, function (err) {
                // console.log("[", socket.user.username, "] is protected for login.");
            });
        }
...
Chat.prototype.join = function (socket, channelName) {
    ...
    var moderator = socket.user && socket.user.admin;

    // Check channelName variable and avoid users to join the mods channel
    if (isChannelNameInvalid(channelName) || isChannelNameModsOnly(channelName)) { return sendError(socket, '[join] Invalid channel name : channelName : ', channelName); }

    // Check if the channel is moderators only
    if (SPECIAL_CHANNELS.hasOwnProperty(channelName)) {
        if (SPECIAL_CHANNELS[channelName].modsOnly && !moderator) { return sendError(socket, '[join] This channel is moderators only'); }
    }
    ...
    if (socket.modCurrentChannel) { socket.leave(socket.modCurrentChannel); }
    ...
    self.getHistory(channelName, socket.user.id, function (err, history) {
...
Chat.prototype.onSay = function (socket, message, isBot, customChannelName) {
    if (!socket.user) { return sendError(socket, '[say] you must be logged in to chat'); }

    if (!socket.currentChannel) { return sendError(socket, '[say] you must be joined before you can chat'); }

    // Check if the message is for a custom channel
    var channelName;
    if (customChannelName) {
        if (isChannelNameInvalid(customChannelName)) { return sendError(socket, '[say] invalid channel name'); } else { channelName = customChannelName; }
    } else { channelName = socket.currentChannel; }

    // Check if the message is for a non writable channel ('all')
    if (SPECIAL_CHANNELS.hasOwnProperty(channelName) && !SPECIAL_CHANNELS[channelName].writable) { return sendErrorChat(socket, 'The all channel is read only'); }
    ...
    if (message.length == 0 || message.length > 500) { return sendError(socket, '[say] invalid message size'); }

    var cmdReg = /^\/([a-zA-z]*)\s*(.*)$/;
    var cmdMatch = message.match(cmdReg);

    if (cmdMatch) // If the message is a command try to execute it
    { this.doChatCommand(socket.user, cmdMatch, channelName, socket); } else // If not broadcast the message
...
Chat.prototype.getHistory = function (channelName, userId, callback) {
    if (channelName === 'all') {
        db.getAllChatTable(CHAT_HISTORY_SIZE, fn);
    } else {
        db.getChatTable(CHAT_HISTORY_SIZE, channelName, userId, fn);
    }
...
Chat.prototype.sendMessageToChannel = function (channelName, msg, userID) {
    ...
    db.canUserChat(userID, function (err) {
        if (err == null) {
            console.assert(msg.hasOwnProperty('bot') && msg.date, msg.hasOwnProperty('message') && msg.type);
            chatter.io.to(channelName).emit('msg', msg);
            chatter.io.to('all').emit('msg', msg);
            chatter.saveChatMessage(userID, msg.message, channelName, msg.bot, msg.date);
        }
    });
};
...
Chat.prototype.saveChatMessage = function (userId, message, channelName, isBot, date) {
    db.addChatMessage(userId, date, message, channelName, isBot, function (err) {
        if (err) { console.error('[INTERNAL_ERROR] got error ', err, ' saving chat message of user id ', userId); }
    });
};
...
Chat.prototype.mute = function (shadow, moderatorUser, username, time, channelName, callback) {
    ...
    db.getUserByName(username, function (err, userInfo) {
        if (err) {
            ...
            { callback(err); } else { console.error('[INTERNAL_ERROR] got error ', err, ' muting user ', userInfo.username); }
            return;
        }
        assert(userInfo);
...
Chat.prototype.unmute = function (moderatorUser, username, channelName, callback) {
    ...
    if (!lib.hasOwnProperty(self.muted, username)) { return callback('USER_NOT_MUTED'); }
